module pyth_navi::governance {
    use pyth_navi::governance_instruction;
    use pyth_navi::governance_action;
    use pyth_navi::set_governance_data_source;
    use pyth_navi::set_data_sources;
    use pyth_navi::set_stale_price_threshold;
    use pyth_navi::set_fee_recipient;
    use pyth_navi::state::{Self, State};
    use pyth_navi::set_update_fee;
    ...
    public fun verify_vaa(
        pyth_navi_state: &State,
        verified_vaa: VAA,
    ): WormholeVAAVerificationReceipt {
        state::assert_latest_only(pyth_navi_state);

        let vaa_data_source = pyth_navi::data_source::new((vaa::emitter_chain(&verified_vaa) as u64), vaa::emitter_address(&verified_vaa));

        // The emitter chain and address must correspond to the Pyth governance emitter chain and contract.
        assert!(
            pyth_navi::state::is_valid_governance_data_source(pyth_navi_state, vaa_data_source),
    ...
    public fun execute_governance_instruction(
        ...
        state::set_last_executed_governance_sequence(&latest_only, pyth_navi_state, sequence);
        ...
        if (action == governance_action::new_contract_upgrade()) {
            ...
            set_governance_data_source::execute(&latest_only, pyth_navi_state, governance_instruction::destroy(instruction));
        } else if (action == governance_action::new_set_data_sources()) {
            set_data_sources::execute(&latest_only, pyth_navi_state, governance_instruction::destroy(instruction));
        } else if (action == governance_action::new_set_update_fee()) {
            set_update_fee::execute(&latest_only, pyth_navi_state, governance_instruction::destroy(instruction));
        } else if (action == governance_action::new_set_stale_price_threshold()) {
            set_stale_price_threshold::execute(&latest_only, pyth_navi_state, governance_instruction::destroy(instruction));
        } else if (action == governance_action::new_set_fee_recipient()) {
            set_fee_recipient::execute(&latest_only, pyth_navi_state, governance_instruction::destroy(instruction));
        } else {
