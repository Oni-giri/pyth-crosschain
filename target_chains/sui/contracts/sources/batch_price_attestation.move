module pyth_navi::batch_price_attestation {
    use std::vector::{Self};
    ...
    fun deserialize_price_info(cur: &mut Cursor<u8>, clock: &Clock): PriceInfo {
        ...
        let current_price = pyth_navi::price::new(price, conf, expo, publish_time);
        if (status != price_status::new_trading()) {
            current_price = pyth_navi::price::new(prev_price, prev_conf, expo, prev_publish_time);
        };
        ...
        price_info::new_price_info(
            ...
            price_feed::new(
                price_identifier,
                current_price,
                pyth_navi::price::new(ema_price, ema_conf, expo, ema_timestamp),
            )
        )
    }
